...
func (s *Signer) write(z *file_nsec3.Zone) error {
...
func write(w io.Writer, z *file_nsec3.Zone) error {
...
// zone (if encountered).
func Parse(f io.Reader, origin, fileName string) (*file_nsec3.Zone, error) {
	zp := dns.NewZoneParser(f, dns.Fqdn(origin), fileName)
	zp.SetIncludeAllowed(true)
	z := file_nsec3.NewZone(origin, fileName)
	seenSOA := false
