...
func (s *Signer) write(z *file_nsec5.Zone) error {
...
func write(w io.Writer, z *file_nsec5.Zone) error {
...
// zone (if encountered).
func Parse(f io.Reader, origin, fileName string) (*file_nsec5.Zone, error) {
	zp := dns.NewZoneParser(f, dns.Fqdn(origin), fileName)
	zp.SetIncludeAllowed(true)
	z := file_nsec5.NewZone(origin, fileName)
	seenSOA := false
