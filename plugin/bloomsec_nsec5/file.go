...
func (s *Signer) write(z *bloomfile_nsec5.Zone) error {
...
func write(w io.Writer, z *bloomfile_nsec5.Zone) error {
...
// zone (if encountered).
func Parse(f io.Reader, origin, fileName string) (*bloomfile_nsec5.Zone, error) {
	zp := dns.NewZoneParser(f, dns.Fqdn(origin), fileName)
	zp.SetIncludeAllowed(true)
	z := bloomfile_nsec5.NewZone(origin, fileName)
	seenSOA := false
