...
func TestFileParse(t *testing.T) {
	...
	tests := []struct {
		...
		{
			`bloomfile ` + zoneFileName1 + ` miek.nl.`,
			false,
			Zones{Names: []string{"miek.nl."}},
		},
		{
			`bloomfile ` + zoneFileName2 + ` dnssex.nl.`,
			false,
			Zones{Names: []string{"dnssex.nl."}},
		},
		{
			`bloomfile ` + zoneFileName2 + ` 10.0.0.0/8`,
			false,
		...
		{
			`bloomfile ` + zoneFileName1 + ` example.net. {
				no_reload
			}`,
			true,
			Zones{},
